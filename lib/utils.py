...
def query_zls(zig_version):
    ...
    encoded_version = quote(zig_version, safe='')
    url = f'https://releases.zigtools.org/v1/zls/select-version?zig_version={encoded_version}&compatibility=full'
...
def download(version, zig_outfile, zls_outfile):
    ...
    # Determine the archive extension from the tarball URL
    ...
    # Append the proper extension to the output files
    zig_outfile_with_ext = zig_outfile + archive_ext
    zls_outfile_with_ext = zls_outfile + archive_ext

    download_tarball(zig_outfile_with_ext, tarball_info, version=version)

    # Print the archive extension so the download script knows how to extract
    print(f'ARCHIVE_EXT={archive_ext}')
    ...
    try:
        ...
        download_tarball(
            zls_outfile_with_ext, tarball_info, version=version, use_mirror=False
        )
    except Exception as e:
